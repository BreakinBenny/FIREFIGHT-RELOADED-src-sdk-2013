...
void CC_Compatibility(void)
{
	...
	if (incompatible)
	{
		...
		Warning("$includemodel \"police_animations_noconflicts.mdl\"\n");
		Warning("\nFemale Model:\n");
	...
	else
	{
		...
		if (iShouldUsePlayersModel == 1 && npc_playerbot_useplayersmodel.GetBool())
		{
			...
			if (pLocalPlayer)
			{
				...
				if (!NPCAnim)
				{
					Warning("npc_player: Using pre-selected model. Use debug_playerbot_check_incompatible_models for more info.\n");
