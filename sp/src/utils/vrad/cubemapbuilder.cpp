...
void BuildCubemaps(bool bHdrMode)
{
	...
	const char* GameExecutableName = GameInfoKVCubemap->GetString("GameExecutableName", NULL);

	if (!GameExecutableName)
	{
		Error("Could not locate '%s' key %s in %s\n", "GameExecutableName", GameExecutableName, GameInfoPath);
	}
